function [data_mont, sc, fs] = read_data_montage(filename)
    ...
    sc = scle(1);  % Still return for compatibility, though no longer used
    ...
    for jj = 1:18
        ...
        elseif length(dat{qq1}) ~= length(dat{qq2})
            data_mont{jj} = zeros(1, length(dat{1}));
        else
            % Convert to double and scale individually before subtraction
            sig1 = double(dat{qq1}) * scle(qq1);
            sig2 = double(dat{qq2}) * scle(qq2);
            data_mont{jj} = sig1 - sig2;  % Result in µV
        end
    end
end
